...
class ShooterXSupabaseUser extends BaseAuthUser {
  ShooterXSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> shooterXSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = ShooterXSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
